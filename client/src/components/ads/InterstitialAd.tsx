import React, { useState, useEffect } from 'react';
import CapacitorAdMobService from '@/services/capacitor-admob';
import { motion, AnimatePresence } from 'framer-motion';

interface InterstitialAdProps {
  isOpen: boolean;
  onClose: () => void;
}

const InterstitialAd: React.FC<InterstitialAdProps> = ({
  isOpen,
  onClose,
}) => {
  const [isVisible, setIsVisible] = useState(false);

  useEffect(() => {
    setIsVisible(isOpen);
  }, [isOpen]);

  const handleComplete = () => {
    setIsVisible(false);
    setTimeout(() => {
      onClose();
    }, 300);
  };

  // Auto-close after 3 seconds (simulates real ad behavior)
  useEffect(() => {
    if (isVisible) {
      const timer = setTimeout(() => {
        handleComplete();
      }, 3000);
      
      return () => clearTimeout(timer);
    }
  }, [isVisible]);

  // Web preview for testing - shows only in web environment
  return (
    <AnimatePresence>
      {isVisible && (
        <motion.div
          initial={{ opacity: 0, scale: 0.8 }}
          animate={{ opacity: 1, scale: 1 }}
          exit={{ opacity: 0, scale: 0.8 }}
          transition={{ duration: 0.3 }}
          className="fixed inset-0 bg-black/90 flex items-center justify-center z-50"
        >
          <motion.div
            initial={{ y: 50, opacity: 0 }}
            animate={{ y: 0, opacity: 1 }}
            exit={{ y: 50, opacity: 0 }}
            transition={{ duration: 0.3, delay: 0.1 }}
            className="bg-white rounded-lg max-w-md mx-4 shadow-2xl relative"
          >
            {/* Close Button (X) */}
            <button
              onClick={handleComplete}
              className="absolute top-2 right-2 w-8 h-8 bg-gray-200 hover:bg-gray-300 rounded-full flex items-center justify-center text-gray-600 hover:text-gray-800 transition-colors z-10"
            >
              ✕
            </button>
            
            {/* Ad Content */}
            <div className="bg-gradient-to-r from-blue-500 to-purple-600 text-white p-8 rounded-lg">
              <h2 className="text-xl font-bold mb-4">📺 Interstitial Advertisement</h2>
              <p className="text-sm opacity-90 mb-4">
                Educational Islamic content for children
              </p>
              <div className="bg-white/20 p-4 rounded text-center">
                <div className="text-2xl mb-2">🎬</div>
                <div className="text-sm">AdMob Test Mode</div>
                <div className="text-xs mt-2 opacity-75">
                  Auto-closing in 3 seconds...
                </div>
              </div>
              <div className="text-xs opacity-75 mt-4">
                In mobile app: Real AdMob interstitial will show
              </div>
            </div>

            {/* Instructions */}
            <div className="p-4 text-center text-sm text-gray-600">
              <div className="mb-2">
                <strong>Development Preview</strong>
              </div>
              <div>
                Native AdMob interstitial ads work automatically in mobile app.
              </div>
            </div>
          </motion.div>
        </motion.div>
      )}
    </AnimatePresence>
  );
};

export default InterstitialAd;